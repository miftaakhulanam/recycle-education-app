...
@section('container')
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
        <div class="container mx-auto px-6 py-8">
            ...
            <div class="mt-8">
                <a href="/dashboard/member/create"
                    ...
                    <div
                        ...
                        <table class="min-w-full">
                            ...
                            <tbody class="bg-white">
                                @php
                                    $iterationNumber = ($user->currentPage() - 1) * $user->perPage();
                                @endphp
                                @foreach ($user as $u)
                                    {{-- Modal delete --}}
                                    <div id="popup-modal-delete-{{ $u->username }}" tabindex="-1"
                                        ...
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <form action="/dashboard/member/{{ $u->username }}" method="POST"
                                                        class="">
                                                        @method('delete')
                                                        @csrf
                                                        <button
                                                    ...
                                                    <button data-modal-hide="popup-modal-delete-{{ $u->username }}"
                                                        type="button"
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 border-b border-gray-200">
                                            <div class="flex items-center justify-center h-14 w-14">
                                                @if (isset($u->photo_profil))
                                                    <img class="h-12 w-12 rounded-full"
                                                        src="{{ asset('storage/' . $u->photo_profil) }}"
                                                        alt="{{ $u->username }}" />
                                                @else
                                                    <img class="h-12 w-12 rounded-full" src="{{ asset('img/profile.png') }}"
                                                        alt="{{ $u->username }}" />
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                            <div class="text-sm leading-5 text-gray-900">{{ $u->email }}
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                            @if ($u->is_admin == 'admin')
                                                <small
                                                    class="uppercase text-[11px] text-center text-white font-medium py-1 px-3 rounded-full bg-green-500">{{ $u->is_admin }}</small>
                                            @else
                                                <small
                                                    class="uppercase text-[11px] text-center text-white font-medium py-1 px-3 rounded-full bg-amber-400">{{ $u->is_admin }}</small>
                                            @endif
                                        </td>
                                        ...
                                        <td
                                            ...
                                            <a href="/dashboard/member/{{ $u->username }}"
                                            ...
                                            <a href="/dashboard/member/{{ $u->username }}/edit"
                                            ...
                                            <button data-modal-target="popup-modal-delete-{{ $u->username }}"
                                                data-modal-toggle="popup-modal-delete-{{ $u->username }}" type="button"
