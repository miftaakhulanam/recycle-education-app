...
@section('container')
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
        <div class="container mx-auto px-6 py-8">
            ...
            <div class="w-[80%]">
                <form method="POST" action="/dashboard/member/{{ $user->username }}" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    <div class="my-5">
                        <label for="username"
                        ...
                        <input type="text" id="username" name="username"
                            ...
                            required autocomplete="off" autofocus value="{{ old('username', $user->username) }}">
                        @error('username')
                    ...
                    <div class="my-5 hidden">
                        ...
                        <input type="text" id="password" name="password"
                            ...
                            required autocomplete="off" value="{{ old('password', $user->password) }}">
                        @error('password')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="my-5">
                        ...
                        <select id="is_admin" name="is_admin"
                            ...
                            <option value="admin" {{ old('is_admin', $user->is_admin) === 'admin' ? 'selected' : '' }}>
                                Admin
                            </option>
                            <option value="member" {{ old('is_admin', $user->is_admin) === 'member' ? 'selected' : '' }}>
                                Member
                            </option>
                        </select>
                    </div>
                    <div class="my-5">
                        ...
                        <input type="text" id="telepon" name="telepon"
                            ...
                            autocomplete="off" value="{{ old('telepon', $user->telepon) }}">
                        @error('telepon')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
